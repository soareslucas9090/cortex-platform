...
- **AlunoCurso**: Tabela de associação (M:N) ligando um `Aluno` a um `Curso`. Possui o atributo opcional `matricula` (identificador acadêmico para login e consultas; unicidade global no sistema).

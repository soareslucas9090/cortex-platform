...
- **Transporte:** capacidade `gerenciar` alinhada a L3 (`is_staff`, `is_admin` ou
  superusuário); capacidade `motorista` para usuários com perfil Motorista ativo
  acessarem a visão somente de leitura das rotas do dia; e capacidade `reservar`
  para aluno ativo, matriculado e com menos de três strikes ativos. Percursos,
  rotas, administração de execuções, ausências, QR Code e análise de justificativas
  são L3; o aluno acessa execuções abertas e recursos próprios de tickets, strikes
  e justificativas. L3 não recebe a capacidade operacional de motorista
  automaticamente.
